...
network* load_model() {
  ...
  for (int i = 1; i < ann->num_of_hidden_layers; i++) {
    ...
    int layer_weights_size =
        the_layer->num_of_inputs * the_layer->num_of_neurons * sizeof(double);
    the_layer->weights = (double*)malloc(layer_weights_size);
    memcpy(the_layer->weights, weights[i], layer_weights_size);
    the_layer->biases =
        (double*)malloc(the_layer->num_of_neurons * sizeof(double));
    memcpy(the_layer->biases, biases[i],
           the_layer->num_of_neurons * sizeof(double));
    the_layer->output =
        (double*)malloc(the_layer->num_of_neurons * sizeof(double));
  ...
  int output_layer_weights_size = output_layer->num_of_inputs *
                                  output_layer->num_of_neurons * sizeof(double);
  output_layer->weights = (double*)malloc(output_layer_weights_size);
  memcpy(output_layer->weights, weights[NUM_OF_HIDDEN_LAYERS],
         output_layer_weights_size);
  output_layer->biases =
      (double*)malloc(output_layer->num_of_neurons * sizeof(double));
  ...
  for (unsigned int l = 0; l < ann->num_of_hidden_layers + 1; l++) {
    ...
    for (unsigned int n = 0; n < ann->layers[l]->num_of_neurons; n++) {
      ...
      for (unsigned int i = 0; i < ann->layers[l]->num_of_inputs; i++) {
        // printf("%f x %f ", previous_output[i],
        // ann->layers[l]->weights[i*ann->layers[l]->num_of_neurons+n]);
        y += previous_output[i] *
             ann->layers[l]->weights[i * ann->layers[l]->num_of_neurons + n];
      }
      // printf("\n");
