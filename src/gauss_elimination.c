...
gauss_real* gauss_elimination(int n,gauss_real **matrix_a,gauss_real *vector_b){
 ...
 for(i=0;i<n;i++) a[i]=(gauss_real*)malloc((n+1)*sizeof(gauss_real));

 x=(gauss_real*)malloc(n*sizeof(gauss_real));

 /* Load the matrix a[][], the matrix a[][] is an n*(n+1) matrix which reads [matrix_a, matrix_b] */
 for(i=0;i<n;i++){
  for(j=0;j<n;j++) a[i][j]=matrix_a[i][j];
  a[i][j]=vector_b[i];
 }
 ...
 while(i<n){
  ...
  /* If any equation does not satisfy the diagonal dominance condition then re-order */
  if(abs(a[i][i])<sum){
   tmp=a[i];
   a[i]=a[indx];
   a[indx]=tmp;
 ...
 for(i=0;i<n-1;i++){
  for(j=i+1;j<n;j++){
   r=a[j][i]/a[i][i];
   for(k=0;k<n+1;k++) a[j][k]=a[j][k]-r*a[i][k];
  }
 }
