...
void monteCarloEstimateStrat(double lowBound, double upBound, int iterations, double statsArray[], int subdomains)
//Function to execute Monte Carlo integration on predefined function, uses stratified sampling of equally sized subdomains
{
	...
	for(int i = 0;i<subdomains;i++)
	{
		...
		STDs[i] = increment*pow( (expectedSquares[i]-pow(expecteds[i],2))/(iterations-1) ,0.5);
