...
/* 
  ...
  */
int poly_interpolation(poly_real x_a[], 
                        poly_real y_a[], 
                        unsigned int n, 
                        poly_real x, 
                        poly_real* y, 
  ...
  *y = py_a[nearest_index--];
  ...
  for (unsigned int column=1; column<n; column++) {
    ...
    /* Decide which correction we want to add to y. The last error is the error indication. */
    *error = (nearest_index << 1) < (n - column) ? c[nearest_index+1] : d[nearest_index--];
    *y += *error;
  }
