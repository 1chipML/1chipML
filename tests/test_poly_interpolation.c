...
void test_poly(poly_real x_a[], poly_real y_a[], unsigned int n, poly_real x, poly_real y_true) {
	poly_real y = 0., error = 0.;

	/* Print solution on screen */
	if (poly_interpolation(x_a, y_a, n, x, &y, &error) == 0 && round(y * 1000) == round(y_true * 1000)) {
		printf("Success: ");
	} else {
		printf("Fail: ");
	}
