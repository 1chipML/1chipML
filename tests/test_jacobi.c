...
int testMaximumIndex(real_number* input, vec_size size, vec_size expectedRow, vec_size expectedCol) {
    ...
    if (maxRow == expectedRow && maxCol == expectedCol) {
        ...
        printf("Fail : Test maximum index. Row should be %d and column should be %d, but row is %d and column is %d\n", expectedRow, expectedCol, maxRow, maxCol);
        return 1;
    }
}
...
int main() {
    ...
    // The numbers are different in the cyclic version since depending on the jacobi rotations,
    // the eigenvectors can be inverted or not.
    real_number expectedEigenVectorsCyclic[9] = {
        -0.541774,  -0.454401,  0.707107,
        -0.642621,  0.766185,   0.0000000,
        -0.541774,  -0.454401,  -0.707107
    };

    fail |= testJacobi(matTestJacobi, expectedEigenValues, expectedEigenVectorsCyclic, 3, 10, 1, EPSILON_CMP);
    fail |= testJacobi(matTestJacobi, expectedEigenValues, expectedEigenVectors, 3, 10, 0, EPSILON_CMP);
